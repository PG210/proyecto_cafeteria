  ...
  class Usuario extends Conexion
  {
    ...
    public function login($user, $clave)
    {
      ...
      */
      ...
      $consulta = 'select idusu, nombre, cargo, telusu, dirusu from usuarios where email="'.$user.'" and clave= MD5("'.$clave.'")';
      /*
    ...
    public function registroUsuario($ced, $name, $email, $clave, $tel, $dir)
    {
      parent::conectar();
      $idusu =parent::filtrar($ced);
      $name  = parent::filtrar($name);
      $email = parent::filtrar($email);
      $clave = parent::filtrar($clave);
      $telusu = parent::filtrar($tel);
      $dirusu = parent::filtrar($dir);

      // validar que el correo no exito
      $verificarCorreo = parent::verificarRegistros('select idusu from usuarios where email="'.$email.'" ');
      ...
      if($verificarCorreo > 0){
        ...
        parent::query('insert into usuarios(idusu, nombre, email, clave, cargo, telusu, dirusu) values("'.$idusu.'","'.$name.'", "'.$email.'", MD5("'.$clave.'"), 2, "'.$telusu.'","'.$dirusu.'")');
